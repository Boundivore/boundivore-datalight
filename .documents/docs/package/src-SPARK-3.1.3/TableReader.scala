    ...
    val unwrappers: Seq[(Any, InternalRow, Int) => Unit] = fieldRefs.map {
      _.getFieldObjectInspector match {
        ...
        case oi: TimestampObjectInspector =>
          (value: Any, row: InternalRow, ordinal: Int) =>
            // DLC-FIX
            val javaSqlTimestamp = oi.getPrimitiveJavaObject(value).toSqlTimestamp
            row.setLong(ordinal, DateTimeUtils.fromJavaTimestamp(javaSqlTimestamp))
            // DLC-FIX
        case oi: DateObjectInspector =>
          (value: Any, row: InternalRow, ordinal: Int) =>
            // DLC-FIX
            val javaSqlDate = new java.sql.Date(oi.getPrimitiveJavaObject(value).toEpochMilli)
            row.setInt(ordinal, DateTimeUtils.fromJavaDate(javaSqlDate))
            // DLC-FIX
