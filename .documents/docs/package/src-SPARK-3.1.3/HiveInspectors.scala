  ...
  def unwrapperFor(objectInspector: ObjectInspector): Any => Any =
    objectInspector match {
      ...
      case poi: WritableConstantDateObjectInspector =>
        val d = poi.getWritableConstantValue.get().toEpochMilli
        // DLC-FIX
        val javaSqlDate = new java.sql.Date(d)
        val constant = DateTimeUtils.fromJavaDate(javaSqlDate)
        // DLC-FIX
        _ => constant
      ...
      case pi: PrimitiveObjectInspector => pi match {
        ...
        case x: DateObjectInspector if x.preferWritable() =>
          data: Any => {
            if (data != null) {
              // DLC-FIX
              val millis = x.getPrimitiveWritableObject(data).get().toEpochMilli
              new java.sql.Date(millis).toLocalDate.toEpochDay.toInt
              // DCL-FIX
            } else {
              null
            }
        ...
        case x: DateObjectInspector =>
          data: Any => {
            if (data != null) {
              // DLC-FIX
              DateTimeUtils.fromJavaDate(x.getPrimitiveJavaObject(data).asInstanceOf[java.sql.Date])
              // DLC-FIX
            } else {
              null
            }
        ...
        case x: TimestampObjectInspector if x.preferWritable() =>
          data: Any => {
            if (data != null) {
              // DLC-FIX
              val javaSqlTimestamp = x.getPrimitiveWritableObject(data).getTimestamp.toSqlTimestamp
              DateTimeUtils.fromJavaTimestamp(javaSqlTimestamp)
              // DLC-FIX
            } else {
              null
            }
        ...
        case ti: TimestampObjectInspector =>
          data: Any => {
            if (data != null) {
              // DLC-FIX
              val javaSqlTimestamp = ti.getPrimitiveWritableObject(data).getTimestamp.toSqlTimestamp
              DateTimeUtils.fromJavaTimestamp(javaSqlTimestamp)
              // DLC-FIX
            } else {
              null
