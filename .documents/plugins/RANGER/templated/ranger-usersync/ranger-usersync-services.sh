...
if [ "${action}" == "START" ]; then
	...
	RangerUserSync_JMX_OPTS=(
      "-Djava.net.preferIPv4Stack=true"
      "-Dcom.sun.management.jmxremote.authenticate=false"
      "-Dcom.sun.management.jmxremote.ssl=false"
      "-Dcom.sun.management.jmxremote.local.only=false"
      "-Dcom.sun.management.jmxremote.port={{jmxRemotePort_RangerUserSync}}"
      "-javaagent:${DATALIGHT_DIR}/exporter/jar/jmx_exporter.jar={{jmxExporterPort_RangerUserSync}}:${SERVICE_DIR}/RANGER/exporter/conf/jmx_config_RangerUserSync.yaml"
  )

  JAVA_OPTS="${JAVA_OPTS} ${RangerUserSync_JMX_OPTS[*]}"
