...
JAVA_OPTS=" ${JAVA_OPTS} -XX:MetaspaceSize=100m -XX:MaxMetaspaceSize=200m -Xmx${ranger_admin_max_heap_size} -Xms1g -Xloggc:/data/datalight/logs/RANGER/gc-worker.log -verbose:gc -XX:+PrintGCDetails"
...
RangerAdmin_JMX_OPTS=(
    "-Djava.net.preferIPv4Stack=true"
    "-Dcom.sun.management.jmxremote.authenticate=false"
    "-Dcom.sun.management.jmxremote.ssl=false"
    "-Dcom.sun.management.jmxremote.local.only=false"
    "-Dcom.sun.management.jmxremote.port={{jmxRemotePort_RangerAdmin}}"
    "-javaagent:${DATALIGHT_DIR}/exporter/jar/jmx_exporter.jar={{jmxExporterPort_RangerAdmin}}:${SERVICE_DIR}/RANGER/exporter/conf/jmx_config_RangerAdmin.yaml"
)
...
start() {
  ...
  JAVA_OPTS="${JAVA_OPTS} ${RangerAdmin_JMX_OPTS[*]}"
