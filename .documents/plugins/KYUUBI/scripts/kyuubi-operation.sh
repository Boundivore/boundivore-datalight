...
# 检查参数是否为空
if [ -z "$1" ]; then
  echo "Usage: $0 <KyuubiServer> <start|stop|restart>"
  exit 1
fi
...
"KyuubiServer")
  case "$1" in
  "start")
    su -c "${CURRENT_SERVICE_DIR}/bin/kyuubi start" "${USER_NAME}"
    ;;
  "stop")
    su -c "${CURRENT_SERVICE_DIR}/bin/kyuubi stop" "${USER_NAME}"
    ;;
  "restart")
    su -c "${CURRENT_SERVICE_DIR}/bin/kyuubi stop" "${USER_NAME}"
    sleep 3
  ...
  *)
  ...
  ;;
*)
  echo "Invalid component name. Supported components: <KyuubiServer>"
  exit 1
  ;;
esac
