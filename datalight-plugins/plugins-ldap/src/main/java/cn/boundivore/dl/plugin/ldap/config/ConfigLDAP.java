 ...
 */
package cn.boundivore.dl.plugin.ldap.config;
 ...
 */
@Slf4j
public class ConfigLDAP extends AbstractConfig {
    /**
    ...
    public String configLogic(File file, String replacedTemplate) {
        switch (file.getName()) {
            case "hdb.ldif":
                return new ConfigLogicHdbLdif(super.pluginConfig).config(file, replacedTemplate);
            case "base.ldif":
                return new ConfigLogicBaseLdif(super.pluginConfig).config(file, replacedTemplate);
            case "rsyslog.conf":
