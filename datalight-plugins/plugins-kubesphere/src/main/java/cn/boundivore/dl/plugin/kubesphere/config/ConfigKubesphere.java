 ...
 */
@Slf4j
public class ConfigKubesphere extends AbstractConfig {
    /**
    ...
    public String configLogic(File file, String replacedTemplate) {
        switch (file.getName()) {
            case "datalight-config-auth-harbor.yaml":
                return new ConfigLogicAuthHarborYaml(super.pluginConfig).config(file, replacedTemplate);
            case "datalight-config-no-auth-harbor.yaml":
                return new ConfigLogicNoAuthHarborYaml(super.pluginConfig).config(file, replacedTemplate);
            default:
