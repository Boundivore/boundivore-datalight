 ...
 */
@Slf4j
public class ConfigSSSD extends AbstractConfig {
    /**
    ...
    public String configLogic(File file, String replacedTemplate) {
        switch (file.getName()) {
            case "sssd.conf":
                return new ConfigLogicSssdConf(super.pluginConfig).config(file, replacedTemplate);
            case "system-auth":
                return new ConfigLogicSystemAuth(super.pluginConfig).config(file, replacedTemplate);
            case ".bashrc":
