 ...
 */
@Slf4j
public class ConfigKYUUBI extends AbstractConfig {
    /**
    ...
    public String configLogic(File file, String replacedTemplate) {
        switch (file.getName()) {
            case "kyuubi-defaults.conf":
                return new ConfigLogicDefaultsConf(super.pluginConfig).config(file, replacedTemplate);
            case "kyuubi-env.sh":
                return new ConfigLogicKyuubiEnvSh(super.pluginConfig).config(file, replacedTemplate);
            case "log4j2.xml":
                return new ConfigLogicLog4j2Xml(super.pluginConfig).config(file, replacedTemplate);
            case ConfigLogicJmxYaml.JMX_CONFIG_FILE_KyuubiServer:
