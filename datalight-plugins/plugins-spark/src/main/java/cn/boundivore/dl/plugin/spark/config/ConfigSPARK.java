...
public class ConfigSPARK extends AbstractConfig {
    ...
    public String configLogic(File file, String replacedTemplate) {
        switch (file.getName()) {
            case "core-site.xml":
                return new ConfigLogicCoreSiteXml(super.pluginConfig).config(file, replacedTemplate);
            case "hdfs-site.xml":
                return new ConfigLogicHdfsSiteXml(super.pluginConfig).config(file, replacedTemplate);
            case "yarn-site.xml":
                return new ConfigLogicYarnSiteXml(super.pluginConfig).config(file, replacedTemplate);
            case "mapred-site.xml":
                return new ConfigLogicMapredSiteXml(super.pluginConfig).config(file, replacedTemplate);
