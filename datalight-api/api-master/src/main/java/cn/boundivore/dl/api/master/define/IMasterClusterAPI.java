 ...
 * Version: V1.0
 */
@Tag(name = "Master 接口：集群相关", description = "IMasterClusterAPI")
@FeignClient(
...
public interface IMasterClusterAPI {
    ...
    @Operation(summary = "查询集群信息", description = "查询集群信息")
    ...
    @Operation(summary = "查询依赖了指定集群的计算集群信息列表", description = "查询依赖了指定集群的计算集群信息列表")
    Result<AbstractClusterVo.ClusterListVo> getComputeClusterListByRelativeClusterId(
            @Parameter(name = "ClusterId", description = "被依赖集群 ID")
            @RequestParam(value = "ClusterId", required = true)
            Long clusterId
    ) throws Exception;

    @GetMapping(value = "/cluster/getClusterRelative")
    @Operation(summary = "查询当前集群依赖的集群信息", description = "查询当前集群依赖的集群信息")
    Result<AbstractClusterVo.ClusterVo> getClusterRelative(
            @Parameter(name = "ClusterId", description = "当前集群 ID")
            @RequestParam(value = "ClusterId", required = true)
            Long clusterId
    ) throws Exception;

    @PostMapping(value = "/cluster/updateCurrentView")
    @Operation(summary = "更新集群视图标记", description = "更新集群视图标记")
    ...
    @Operation(summary = "移除集群", description = "移除集群")
